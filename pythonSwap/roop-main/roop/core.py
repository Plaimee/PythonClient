...
def parse_args_socket(sourcePath, targetPath) -> None:
    outputFolder = "E:\\work\\ADI\\SocketIO\\outputImg\\"
    now = datetime.now()
    outputPath = outputFolder + now.strftime("%d_%m_%Y_%H%M%S") + ".png"
    roop.globals.source_path = sourcePath
    roop.globals.target_path = targetPath
    roop.globals.output_path = normalize_output_path(roop.globals.source_path, roop.globals.target_path, outputPath)
    roop.globals.headless = roop.globals.source_path is not None and roop.globals.target_path is not None and roop.globals.output_path is not None
...
def start_socket() -> str:
    ...
    if has_image_extension(roop.globals.target_path):
        ...
        if is_image(roop.globals.target_path):
            update_status('Processing to image succeed!')
            output = roop.globals.output_path
        else:
            update_status('Processing to image failed!')
    # clean temp
    update_status('Cleaning temporary resources...')
...
def run_socket(sourcePath, targetPath) -> str:
    output = ""
    parse_args_socket(sourcePath, targetPath)
    if not pre_check():
        return
    for frame_processor in get_frame_processors_modules(roop.globals.frame_processors):
        if not frame_processor.pre_check():
